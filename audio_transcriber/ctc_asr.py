...
class AudioTranscriber:
    ...
    def transcribe_with_timestamps(self, audio_path: Path) -> list[dict]:
        """CTC version with improved timestamp estimation."""
        audio, sr = sf.read(audio_path, dtype="float32", always_2d=True)
        audio = audio[:, 0]  # Select first channel
        ...
        # Get frame-level predictions and logits
        outputs = self.session.run(None, input_dict)
        logits = outputs[0]
        frame_predictions = np.argmax(logits, axis=-1)[0]
        
        # Calculate frame duration in seconds
        frame_duration = self.melspectrogram.hop_length / self.melspectrogram.sample_rate
    ...
    def _estimate_ctc_timestamps(self, frame_predictions):
        """Estimate timestamps from CTC alignment."""
        # Implement CTC-specific timing logic
        # This is simplified - you'll need proper frame-to-time conversion
        timestamps = []
        current_word = []
        start_time = 0.0
        frame_duration = 0.02  # Example: 20ms per frame
        
        for i, token_id in enumerate(frame_predictions):
            token = self.idx2token.get(int(token_id), "")
            if token and token != "<blank>":
                if not current_word:  # Start new word
