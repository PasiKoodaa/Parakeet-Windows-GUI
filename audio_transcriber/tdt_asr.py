...
class AudioTranscriber:
    ...
    def transcribe_with_timestamps(self, audio_path: Path) -> list[dict]:
        ...
        # Calculate important timing parameters
        original_duration = len(audio) / sr
        hop_length = self.melspectrogram.hop_length
        frame_duration = hop_length / sr  # Duration of each frame before encoder
        
        features = self._process_audio(audio)
        encoder_out = self.model.run_encoder(features)
        
        # Calculate encoder time reduction factor
        input_frames = features.shape[2]  # Number of frames going into encoder
        output_frames = encoder_out.shape[2]  # Number of frames from encoder
        time_reduction_factor = input_frames / output_frames
        
        # Adjusted frame duration after encoder time reduction
        adjusted_frame_duration = frame_duration * time_reduction_factor
        ...
        # Initialize decoder
        state0, state1 = self.model.get_decoder_initial_state()
        last_emitted_token = self.blank_id
        decoder_out, next_state0, next_state1 = self.model.run_decoder(last_emitted_token, state0, state1)
        
        current_t = 0
        max_encoder_t = encoder_out.shape[2]
        
        while current_t < max_encoder_t:
            encoder_out_t = encoder_out[:, :, current_t:current_t+1]
            joiner_logits = self.model.run_joiner(encoder_out_t, decoder_out)
            ...
            duration_bin = np.argmax(duration_logits)
            skip_frames = self.tdt_durations[duration_bin]
            
            if predicted_token != self.blank_id:
                token_text = self.idx2token[predicted_token]
                
                # Calculate absolute times
                start_time = current_t * adjusted_frame_duration
                end_time = (current_t + skip_frames) * adjusted_frame_duration
                
                # Handle word boundaries
                if token_text.startswith("▁"):
                    if current_word:  # Finish previous word
                ...
                # Update decoder
                last_emitted_token = predicted_token
                state0, state1 = next_state0, next_state1
                decoder_out, next_state0, next_state1 = self.model.run_decoder(
                    last_emitted_token, state0, state1)
            
            current_t += skip_frames
        ...
        if current_word:
            word_timestamps.append({
                'word': ''.join(current_word).replace("▁", " ").strip(),
                'start': current_start,
                'end': current_t * adjusted_frame_duration
            })
        
        # Verify timestamps cover full audio duration
        if word_timestamps and word_timestamps[-1]['end'] < original_duration:
            word_timestamps[-1]['end'] = original_duration
        
        # Create segments (same as before)
        segments = self._create_natural_segments(word_timestamps)
    ...
    def _create_natural_segments(self, word_timestamps, max_segment_length=10.0):
        """Group words into natural segments based on pauses and length."""
        segments = []
        current_segment = []
        last_end = 0.0
        ...
        for word in word_timestamps:
            ...
            # 1. Pause > 1 second since last word
            # 2. Current segment would exceed max length
            # 3. Word ends with punctuation
            if (current_segment and 
                ((word['start'] - last_end > 1.0) or
                (word['end'] - current_segment[0]['start'] > max_segment_length) or
                word['word'][-1] in '.!?')):
                
                segments.append({
                    'segment': ' '.join(w['word'] for w in current_segment),
    ...
    def _estimate_word_timestamps(self, token_ids, encoder_out):
        """Estimate word timestamps based on duration predictions."""
        # Implement based on your model's duration predictions
        # This is a placeholder implementation
        timestamps = []
        current_time = 0.0
        word_duration = 0.5  # Example fixed duration - replace with actual prediction
        
        for token in token_ids:
            word = self.idx2token.get(token, "")
    ...
    def _group_into_segments(self, word_timestamps):
        """Group words into logical segments."""
        # Simple implementation - group every 5 words
        segments = []
        current_segment = []
        
        for i, word in enumerate(word_timestamps):
            current_segment.append(word)
            if (i + 1) % 5 == 0 or i == len(word_timestamps) - 1:
                segments.append({
                    'segment': " ".join(w['word'] for w in current_segment),
                    'start': current_segment[0]['start'],
                    'end': current_segment[-1]['end']
    ...
    def _estimate_char_timestamps(self, word_timestamps):
        """Estimate character-level timestamps."""
        char_timestamps = []
        for word in word_timestamps:
